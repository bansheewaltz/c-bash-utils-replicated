...
static void validate_option(s_options *flags, char arg[], char **invalid_opt, int *invalid_len) {
    if (*arg == '-' && *arg++) {
...
bool parse_options(int argc, char *argv[], s_options *flags, int *filename_was_given) {
    ...
    sort_argv(argc, argv, filename_was_given);  // in a case if there are some options
                                                // that were passed after filenames (GNU feature)
    for (int arg_ndx = 1; arg_ndx < argc && *argv[arg_ndx] == '-' && !invalid_opt; ++arg_ndx) {
